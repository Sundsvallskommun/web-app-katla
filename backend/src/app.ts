...
const samlStrategy = new Strategy(
  ...
  async function (profile: Profile, done: VerifiedCallback) {
    ...
    const groups = Array.isArray(profile['http://schemas.xmlsoap.org/claims/Group'])
      ? profile['http://schemas.xmlsoap.org/claims/Group'].join(',')
      : profile['groups'];
    ...
    const groupList: string[] = typeof groups === 'string' ? groups.split(',').map(x => x.toLowerCase()) : [];
